  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', minHeight: '100vh', background: '#f8f9fc' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {['Home', 'Talk to AI', 'Schemes', 'Scan ID', 'Dashboard'].map((tab, i) => (
            <button key={tab}
              onClick={() => i === 0 ? navigate('/') : i === 1 ? navigate('/chat') : i === 2 ? navigate('/schemes') : i === 4 ? navigate('/dashboard') : null}
              style={{
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 5%' }}>
        <h1 style={{ fontSize: 30, fontWeight: 800, color: '#1a1a2e', margin: '0 0 8px' }}>📄 Scan Your ID Document</h1>
        <p style={{ fontSize: 15, color: '#888', margin: '0 0 32px' }}>Upload your Aadhaar card or Ration card — we'll extract your details automatically and match you to the right schemes.</p>
        ...
        <div
          ...
          <p style={{ fontSize: 14, color: '#888', margin: 0 }}>
            {file ? 'Click to change file' : 'or click to browse — Aadhaar, Ration Card, PAN'}
          </p>
        ...
        <button
          ...
          {loading ? '⏳ Extracting details...' : '🔍 Extract & Find Schemes'}
        </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginTop: 32 }}>
          {[
            { icon: '🔒', title: 'Secure', desc: 'Your document is never stored — only the extracted text is used.' },
            { icon: '⚡', title: 'Instant', desc: 'AI extracts your details in under 3 seconds.' },
            { icon: '🎯', title: 'Accurate', desc: 'Auto-fills your profile for better scheme matching.' },
          ].map((c, i) => (
